const authenticate = async (req, res, next) => {
  ...
  if (jwttoken === "")
    // check token is not available then through an error
    return res.status(400).send({
      trackId,
      message: "one-time password incorrect or already uesed it ",
    });
  if (jwttoken === "undefined")
    // check token is undefined  then through an error Authorization token not found or incorrect
    return res.status(400).send({
      trackId,
      message: "Authorization token not found or incorrect....!....",
    });
  if (!jwttoken)
    // if token is not available then through an error Authorization token not found or incorrect
    return res
      .status(400)
